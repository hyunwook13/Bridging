            ...
            let listener = self.db
                ...
                .addSnapshotListener { snapshot, error in
                    ...
                    for change in changes where change.type == .added {
                        do {
                            var post = try change.document.data(as: PostDTO.self)
                            post.uuid = change.document.documentID
                            post.isTemporaryFlag = true
                            
                            let newPost = post.toEntity()
...
//        }
//    }
    public func saveComment(with postUUID: String?, comment: Comment) -> Single<Comment> {
        return Single.create { comp in
            ...
            do {
                let commentDTO = CommentDTO(from: comment)
                let _ = try self.db
                .collection("posts")
                .document(postUUID)
                .collection("comments")
                .addDocument(from: commentDTO)
    ...
    public func fetchComments(with postUUID: String?) -> Single<[Comment]> {
        return Single.create { comp in
            guard let postUUID = postUUID else {
                comp(.failure(NSError(domain: "", code: 0)))
                return Disposables.create()
            ...
            Task {
                do {
                    ...
                    for snapshot in docRef.documents {
                        ...
                        if let entity = comment.toEntity() {
        ...
        return Single.create { single in
            ...
            docRef.getDocument { snapshot, error in
                if let error = error {
                    ...
                    do {
                        let post = try snapshot?.data(as: PostDTO.self)
                        let entityPost = post?.toEntity()
                        single(.success(entityPost))
                    } catch {
                        single(.failure(error))
                    }
                }
            ...
            query.getDocuments { snapshot, error in
                if let error = error {
                    ...
                    let newPosts = snapshot.documents.compactMap {
                        var data = try? $0.data(as: PostDTO.self)
                        data?.uuid = $0.documentID
                        data?.isTemporaryFlag = true
                        let newPost = data?.toEntity()
                        return newPost
                    }
    ...
    public func saveVote(postUUID: String?, type: VoteType) -> Completable {
        Completable.create { comp in
            ...
            let vote = VoteDTO(vote: type, ageGroup: profile.ageGroup, createdAt: Timestamp(date: Date()))
            do {
    ...
    private func saveProfile(uid: String, profile: UserProfile) -> Completable {
        Completable.create { single in
            do {
                var profileDTO = UserProfileDTO(from: profile)
    ...
    public func fetchPosts(withTitle title: String,
        ...
        var posts = snapshot.documents
            .compactMap { try? $0.data(as: PostDTO.self) }
            .compactMap { $0.toEntity() } // ⭐ DTO → Entity
        ...
        return Single.create { single in
            ...
            docRef.getDocument { snap, error in
                if let error = error {
                    ...
                    do {
                        let post = try snap?.data(as: PostDTO.self)
                        single(.success(post?.toEntity()))
                    } catch {
                        single(.failure(error))
                    }
                }
        ...
        return Single.create { single in
            ...
            votesRef.getDocuments { snap, error in
                if let error = error {
                    ...
                    let votes: [Vote] = snap?.documents.compactMap {
                        guard let dto = try? $0.data(as: VoteDTO.self) else { return nil }
                        let vote = dto.toEntity()  // 또는 Vote(from: dto)
                        return vote
                    } ?? []
